...
{loading ? <Loader />:
  ...
  <h2 className='text-2xl text-blue-700 font-bold p-8 tracking-[2px]' style={{fontFamily: 'arial'}}>The popular cars booking offers</h2><div className='mt-10'>
                ...
                <input
                  ...
                  className='w-[60%] h-[5vh] mt-4 shadow-2xl border  rounded-l-lg outline-blue-700 text-gray-400'
                  value={value}
                  onChange={(e) => setValue(e.target.value)} />

                {/* قائمة التبديل */}
                <select
                  className='w-[15%] h-[5vh] mt-4 shadow-2xl border-transparent text-center rounded-r-lg bg-blue-600 text-white '
                  value={mood}
    ...
    <div className="w-full sm:w-full md:w-full lg:w-full xl:w-[80%] m-auto">
          ...
          <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 p-10 gap-4 justify-items-center place-items-center' data-aos="fade-up" >
            {currentData.map((el, index) => (
              <div key={el.id || index}  className={`flex flex-col text-center mb-2 shadow-2xl w-[95%] gap-2 justify-between  ${isDarkMode ? 'bg-gray-800 text-white' : 'bg-white'} `} >
                
                <img
                  src={el.img[0]}
                  alt={el.car}
                  className="w-full h-60 mb-4 m-auto shadow-lg"
                />
                {
                  el.isBooked === true ? (
                    <p className='text-center text-lg font-semibold bg-red-600 text-white'>Booked</p>
                  ) : (
