...
<h1 className='font-semibold text-4xl mt-24 mb-10'>Tweets of some users</h1>
        <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3'>

{Names.map((item) => (
    <div key={item.id} className='grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-2 border border-gray-300 rounded-lg  shadow-xl ml-[5%] mb-4'data-aos="zoom-in">

<div>

    <img src={item.image} alt="image" className='w-80 h-full m-auto rounded-l-lg  hover:scale-110'  />
    </div>
